<template>
    <div class="leading-tight">
        <ul v-if="hasResource" class="list-reset flex flex-wrap justify-between">
            <li class="mb-3 w-full" style="height:100px;" v-for="resource in selections" :key="resource.id">
                <selected-product-variant-list-item :channelId="channelId" :resource="resource"></selected-product-variant-list-item>
            </li>
        </ul>
        <div v-else class="card-no-shadow flex flex-col items-center w-full justify-center" style="min-height: 200px">
...
<script>
  export default {
    name: 'selected-product-variant-list',
    ...
    computed: {
      hasResource () {
        return (_.get(this, 'selections', [])).length > 0
      },
    }
